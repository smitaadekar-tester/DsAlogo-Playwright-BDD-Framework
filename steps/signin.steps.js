...
When('The user enters signin username {string}', async ({ signInPage }, username) => {
  await signInPage.enterUsername(username);
});

When('The user enters signin password {string}', async ({ signInPage }, password) => {
  await signInPage.enterPassword(password);
});

// With Excel data
When('The user enters signin credentials from {string} and {int}', 
...
When('The user clicks Register link from signin page', async ({ signInPage }) => {
...
Then('The user sees signin error message {string}', async ({ signInPage, page }, expectedError) => {
  ...
  if (expectedError === 'Please fill out this field') {
    ...
    if (usernameValidation && usernameValidation.length > 0) {
      ...
      const passwordValidation = await page.locator('#id_password, input[name="password"]')
        .evaluate((el) => el.validationMessage);
...
Then('The user should be redirected to login page', async ({ page }) => {
